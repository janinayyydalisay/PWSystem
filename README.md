...
## Plants dashboard and API

This project includes a simple Plants dashboard and REST API backed by Firestore. Files added/updated:

- `models/plantModel.js` — Firestore access functions for plants
- `routes/plantRoutes.js` — REST API for CRUD (mounted at `/api/plants`)
- `views/dashboard.xian` — UI table, modal form, and client JS to call the API
...
Ensure the Firebase service account JSON (`models/pws4f2-firebase-adminsdk-fbsvc-1b151b8847.json`) exists and the app can access Firestore.
